...
- can ask AI to draft or refine a normalized architecture before YAML is generated,
...
- optional `ai-architecture.json`: the structured AI architecture draft used before deterministic YAML generation
...
To let AI create a normalized architecture from notes and then have this app generate the Threagile YAML from its template:

```bash
export OPENAI_API_KEY="your_api_key_here"
threatmod --ai-architecture --architecture-notes "Driver uses a mobile app over HTTPS to reach an OEM backend. The backend stores telemetry and sends OTA update metadata." --output-dir output
```

You can also combine a source file with AI refinement:

```bash
export OPENAI_API_KEY="your_api_key_here"
threatmod examples/vehicle_gateway.puml --ai-architecture --architecture-notes "Add any implied diagnostics, logging, storage, and update paths that are missing but clearly needed." --output-dir output
```
...
- choose between direct import, AI draft from description, or import plus AI draft
- enter architecture notes for AI-assisted model creation
...
The AI architecture workflow is intentionally structured:

- AI returns `title`, `components`, `trust_boundaries`, and `data_flows` as validated JSON.
- The app normalizes aliases, fills any missing flow endpoints as review-needed components, and writes `ai-architecture.json`.
- The final `threagile-model.yaml` and `threagile.yaml` are still generated by the local Threagile template builder and YAML serializer.
...
- [src/threatmod_automation/ai_review.py](/home/aldo/Documents/Threat_Modelling/src/threatmod_automation/ai_review.py): optional AI architecture drafting plus OpenAI and GitHub Copilot architecture reviewers
